...
class Consumer:
    ...
    def start_consuming(self):
        os.system("ssh %s@%s 'supervisorctl start stream_analysis%d_%d'" % (self.user, self.ip, self.unique_id,
                                                                            self.worker_number))
        os.system("ssh %s@%s 'supervisorctl start cpu_usage%d'" % (self.user, self.ip, self.unique_id))

    def stop_consuming(self):
        os.system("ssh %s@%s 'supervisorctl stop stream_analysis%d_%d'" % (self.user, self.ip, self.unique_id,
                                                                           self.worker_number))
        os.system("ssh %s@%s 'supervisorctl stop cpu_usage%d'" % (self.user, self.ip, self.unique_id))
    ...
    def ping(self):
        os.system("ssh %s@%s 'supervisorctl status stream_analysis%d_%d'" % (self.user, self.ip, self.unique_id,
                                                                             self.worker_number))
        os.system("ssh %s@%s 'supervisorctl status cpu_usage%d'" % (self.user, self.ip, self.unique_id))
    ...
    def gather_report(self):
        ...
        os.system("scp %s@%s:/var/log/supervisor/cpu_usage%d.out cpu_usage_worker%d.out" %
                  (self.user, self.ip, self.unique_id, self.unique_id))
    ...
    def clear_report(self):
        ...
        os.system("ssh %s@%s 'rm /var/log/supervisor/cpu_usage%d.out'" % (self.user, self.ip, self.unique_id))
